...
def generate_neighbors(path):
    ...
    : return dict_of_neighbors: Dictionary with key each node and value a list of lists with the neighbors of the node and the cost (distance) for each neighbor.
    ...
    with open(path) as f:
        for line in f:
            if line.split()[0] not in dict_of_neighbors:
                _list = []
                _list.append([line.split()[1], line.split()[2]])
                dict_of_neighbors[line.split()[0]] = _list
            else:
                dict_of_neighbors[line.split()[0]].append(
                    [line.split()[1], line.split()[2]]
                )
            if line.split()[1] not in dict_of_neighbors:
                _list = []
                _list.append([line.split()[0], line.split()[2]])
                dict_of_neighbors[line.split()[1]] = _list
            else:
                dict_of_neighbors[line.split()[1]].append(
...
def main(args=None):
    dict_of_neighbours = generate_neighbors(args.File)
