...
class Graph:
    ...
    def print_solution(self, distances_from_source: list[int]) -> None:
        ...
        for vertex in range(self.vertices):
            print(vertex, "\t\t", distances_from_source)
    
    def mminimum_distance(
        ...
        # Search not nearerst vertex not in the shortest path tree
        for vertex in range(self.vertices):
    ...
    def dijkstra(self, source: int) -> None:
        ...
        for _ in range(self.vertices):
            u = self.mminimum_distance(distances, visited)
            visited[u] = True
