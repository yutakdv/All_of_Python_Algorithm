"""
Breadth first Search (BFS) can be used when finding the shortest path from a given source
node to a target node in an unweighted graph.
"""
...
class Graph:
    def __init__(self, graph: dict[str, list[str]], source_vertex: str) -> None:
        """
        Graph is implemented as dictionary of adjacency lists. Also, Source vertex have to be
    ...
    def breadth_first_search(self) -> None:
        """
        This function is a helper for running breadth first search on this graph.
    ...
    def shortest_path(self, target_vertex: str) -> str:
        ...
        if target_vertex_parent is None:
            raise ValueError(f"No path from vertex: {self.source_vertex} to vetex: {target_vertex}")
